...
{
    public class ActStaminaDrain : Spell
    {
        ...
        public override bool Perform()
        {
            ...
            try
            {
                ...
                int available = Math.Max(0, target.stamina.value);
                ...
                LangHelper.Say("castStaminaDrain", caster, Act.TC);

                if (amount > 0)
                {
                    NecroVFX.PlayDrain(target, caster, NecroVFX.DrainAmber);
                    target.stamina.Mod(-amount);
                    caster.stamina.Mod(amount / 2);
                }
            }
            catch (Exception ex)
            {
                ModLog.Warn($"ActStaminaDrain.Perform failed: {ex.Message}");
            }
