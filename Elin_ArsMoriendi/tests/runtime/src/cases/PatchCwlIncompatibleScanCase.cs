...
// This invokes CWL TestIncompatibleIl directly and can destabilize runtime in some environments,
// so this case is intended for explicit compat runs, not default smoke runs.
public sealed class ZzzPatchCwlIncompatibleScanCase : RuntimeCaseBase
{
    private const string ArsPatchTypePrefix = "Elin_ArsMoriendi.Patch_";
    ...
    public override void Prepare(RuntimeTestContext ctx)
    {
        ...
        var patchMethods = CollectArsPatchMethods();
        RuntimeAssertions.Require(patchMethods.Count > 0, "No Ars Harmony patch methods were discovered.");
    ...
    private static List<MethodInfo> CollectArsPatchMethods()
    {
        ...
        HashSet<string> seenKeys)
    {
        ...
        foreach (var patch in enumerable)
        {
            ...
            if (!fullName.StartsWith(ArsPatchTypePrefix, StringComparison.Ordinal))
                continue;
