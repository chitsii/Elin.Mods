...
// Active CWL compatibility scan for quest patch methods.
// This is excluded from default smoke and should be run by explicit CaseId.
public sealed class ZzzPatchCwlIncompatibleScanCase : RuntimeCaseBase
{
    private const string PatchTypePrefix = "Elin_QuestMod.Patches.Patch_";

    public override string Id => "questmod.patch.compat.cwl_incompatible_scan";
    ...
    public override void Prepare(RuntimeTestContext ctx)
    {
        ...
        var patchMethods = CollectPatchMethods();
        RuntimeAssertions.Require(patchMethods.Count > 0, "No quest patch methods were discovered.");
    ...
    public override void Execute(RuntimeTestContext ctx)
    {
        ...
        {
            MethodInfo patchMethod = patchMethods[i];
            if (patchMethod == null)
            {
                continue;
            }
    ...
    public override void Verify(RuntimeTestContext ctx)
    {
        ...
        ctx.Log("CWL active incompatible scan passed.");
    }

    private static List<MethodInfo> CollectPatchMethods()
    {
        ...
        foreach (var target in patchedMethods)
        {
            ...
            object patchInfo = HarmonyCompatFacade.GetPatchInfo(target);
            if (patchInfo == null)
            {
                continue;
            }
    ...
    {
        object patchList = GetPatchBucket(patchInfo, bucket);
        ...
        {
            MethodInfo patchMethod = GetPatchMethod(patch);
            if (patchMethod == null)
            {
                continue;
            }
    ...
    private static object GetPatchBucket(object patchInfo, string bucket)
    {
        ...
        object known = HarmonyCompatFacade.GetPatchBucket(patchInfo, bucket);
        if (known != null)
        {
            return known;
        }
    ...
    private static MethodInfo GetPatchMethod(object patch)
    {
        ...
        object methodObj = ReflectionCompat.GetFieldOrPropertyValue(patch, "PatchMethod", "patchMethod", "method");
