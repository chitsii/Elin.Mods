...
class ChoiceReaction:
    ...
    def complete_quest(
        ...
        """クエストを完了（CWL/vanilla標準 action: completeQuest）。"""
        return self._add_action(
            {
...
class DramaBuilder:
    ...
    def add_temp_actor(self, actor: Union[str, DramaActor]) -> "DramaBuilder":
        """既存 Chara を参照する一時アクターを追加（addTempActor）。"""
        actor_key = self._resolve_key(actor)
        self.entries.append({"action": "addTempActor", "actor": actor_key})
        return self
    ...
    def switch_on_flag(
        ...
        if fallback is not None:
            ...
            # default branch: when no previous case matched, jump to fallback
            self.entries.append({"jump": fallback_key})
    ...
    def invoke_expansion(
        ...
        *args: Union[str, int, float],
        ...
        """CWL invoke* 拡張メソッドを呼ぶ。"""
        arg_text = ", ".join(str(a) for a in args)
        expr = f"{method_name}({arg_text})"
        return self.action("invoke*", param=expr, jump=jump, actor=actor)
    ...
    def move_next_to(
        ...
        """actor を target_actor_id の隣へ移動（CWL scene拡張）。"""
        return self.invoke_expansion("move_next_to", target_actor_id, actor=actor)
    ...
    def move_tile(
        ...
        """actor を相対座標で移動（CWL scene拡張）。"""
        return self.invoke_expansion("move_tile", x_offset, y_offset, actor=actor)
    ...
    def move_to(
        ...
        """actor を絶対座標へ移動（CWL scene拡張）。"""
        return self.invoke_expansion("move_to", x, y, actor=actor)
    ...
    def play_effect_ext(
        ...
        """actor に play_effect(effect_id) を適用（CWL scene拡張）。"""
        return self.invoke_expansion("play_effect", effect_id, actor=actor)
    ...
    def play_emote(
        ...
        """actor に play_emote(Emo[, duration]) を適用（CWL scene拡張）。"""
        if duration is None:
            return self.invoke_expansion("play_emote", emote_id, actor=actor)
        return self.invoke_expansion("play_emote", emote_id, duration, actor=actor)
    ...
    def set_portrait(
        ...
        """actor の立ち絵を変更（CWL scene拡張）。None でデフォルトへ戻す。"""
        if portrait_id is None:
            return self.invoke_expansion("set_portrait", actor=actor)
        return self.invoke_expansion("set_portrait", portrait_id, actor=actor)
    ...
    def set_sprite(
        ...
        """actor のスプライトを変更（CWL scene拡張）。None でデフォルトへ戻す。"""
        if sprite_id is None:
            return self.invoke_expansion("set_sprite", actor=actor)
        return self.invoke_expansion("set_sprite", sprite_id, actor=actor)
    ...
    def spawn_npc(
        ...
        """CWL eval で NPC を現在地近傍へ生成する（modInvoke非依存）。

        actor_alias を指定すると、生成した個体をドラマ actor としてバインドする。
        """
        create_expr = (
            f'CharaGen.Create("{chara_id}", {int(level)})'
            if level is not None
            else f'CharaGen.Create("{chara_id}")'
        )
        bind_expr = ""
        if actor_alias:
            bind_expr = f"""
                var drama = LayerDrama.Instance?.drama;
                if (drama?.sequence != null) {{
                    drama.sequence.AddActor("{actor_alias}", new Person(spawned));
                }}
            """
        code = f"""
            var spawned = {create_expr};
            if (spawned != null && EClass._zone != null && EClass.pc != null && EClass.pc.pos != null) {{
                var spawnPoint = EClass.pc.pos.GetNearestPoint(allowBlock: false, allowChara: false);
                EClass._zone.AddCard(spawned, spawnPoint);
                {bind_expr}
            }}
        """.replace("\n", " ").strip()
    ...
    def show_book(
        ...
        if "/" in normalized:
            # Accept preformatted entries like "Book/quest_intro_guide".
            book_entry = normalized
        else:
            short_id = (
                normalized[:-4] if normalized.lower().endswith(".txt") else normalized
            )
    ...
    def cs_call(
        ...
        *arg_exprs: str,
        ...
        例:
            builder.cs_call("Elin_QuestMod.Drama.DramaRuntime.PlayBgm", '"BGM/MyTrack"')
        """
        args = ", ".join(arg_exprs)
    ...
    def stop_bgm_now(self, actor: Union[str, DramaActor] = None) -> "DramaBuilder":
        """現在のBGMを停止。"""
        # stopBGM is a vanilla drama action
        return self.action("stopBGM")
    ...
    def resolve_cooldown_elapsed_raw(
        self,
        last_raw_flag_key: str,
        target_flag_key: str,
        threshold_raw_minutes: int,
        actor: Union[str, DramaActor] = None,
    ) -> "DramaBuilder":
    ...
    def resolve_cooldown_elapsed_days(
        self,
        last_raw_flag_key: str,
        target_flag_key: str,
        cooldown_days: int = 1,
        ...
        """最終時刻フラグとの差分(日)を判定し、結果を target_flag_key(1/0) に同期する。"""
        return self.resolve_cooldown_elapsed_raw(
            last_raw_flag_key,
            target_flag_key,
            threshold_raw_minutes=max(0, int(cooldown_days)) * 1440,
            actor=actor,
        )

    def stamp_current_raw_time(
        self,
        target_flag_key: str,
        actor: Union[str, DramaActor] = None,
    ) -> "DramaBuilder":
    ...
    def branch_if_all_flags(
        ...
        """複数フラグANDを評価して、成立時に jump_to へ分岐する。"""
        self.resolve_flags_all(source_flags, result_flag_key, actor=actor)
        return self.branch_if(result_flag_key, "==", 1, jump_to, actor=actor)

    def branch_if_cooldown_elapsed_days(
        self,
        last_raw_flag_key: str,
        cooldown_days: int,
        result_flag_key: str,
        ...
        """時間経過(日)を評価して、成立時に jump_to へ分岐する。"""
        self.resolve_cooldown_elapsed_days(
            last_raw_flag_key,
            result_flag_key,
            cooldown_days,
            actor=actor,
    ...
    def set_background(self, bg_id: str = "") -> "DramaBuilder":
        """背景画像を設定（CWL/vanilla 標準 action: setBG）。空文字で解除。"""
        self.entries.append({"action": "setBG", "param": bg_id or ""})
        return self

    def clear_background(self) -> "DramaBuilder":
        """背景画像を解除する。"""
        self.entries.append({"action": "setBG", "param": ""})
        return self
    ...
    def multi_branch(
        ...
        使用例:
            builder.multi_branch(
                label=rank_check,
                flag="yourname.elin_quest_mod.flag.rank",
                branches={
    ...
    def start_quest(
        ...
        """クエストを開始（CWL/vanilla標準 action: startQuest）。"""
    ...
    def complete_quest(
        ...
        """クエストを完了（CWL/vanilla標準 action: completeQuest）。"""
        self.entries.append({"action": "completeQuest", "param": quest_id or ""})
        return self
