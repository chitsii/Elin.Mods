...
namespace Elin_SukutsuArena.Conditions
{
    ...
    public class ConAstarothDeletion : Condition
    {
        ...
        private void ApplyDamage()
        {
            ...
            int before = owner.tempElements.Base(ELE_MAG);
            owner.tempElements.ModBase(ELE_MAG, -1000);
            int after = owner.tempElements.Base(ELE_MAG);
            int actualDamage = before - after;

            if (actualDamage > 0)
            {
                string statName = EClass.sources.elements.map[ELE_MAG].GetName();
