...
Generates strongly-typed C# classes from arena.data.
...
def generate_csharp() -> str:
    ...
    lines.append("// Generated by generate_flags.py")
    lines.append("// Do not edit manually. Edit tools/arena/data/config.py instead.")
    lines.append("// </auto-generated>")
    lines.append("")
    ...
    for enum_type in get_all_enums():
        lines.append(
            f"    /// <summary>{enum_type.__doc__ or enum_type.__name__}</summary>"
        )
    ...
    lines.append(f'        public const string QuestDonePrefix = "{QUEST_DONE_PREFIX}";')
    lines.append("")
    ...
    for flag in get_all_flags():
        if flag.key.startswith("player."):
            ...
            lines.append(
                f'        public const string {const_name} = "{flag.full_key}";'
            )
    lines.append("        #endregion")
    lines.append("")
    ...
    # Generate Session Flag Keys
    lines.append("    /// <summary>")
    lines.append(f"    /// セッションフラグキー定数クラス (Prefix: {SESSION_PREFIX})")
    lines.append("    /// </summary>")
    lines.append("    public static class SessionFlagKeys")
    lines.append("    {")
    lines.append(f'        public const string Prefix = "{SESSION_PREFIX}";')
    lines.append("")

    for session_flag in get_all_session_flags():
        const_name = to_pascal_case(session_flag.key)
        lines.append(f"        /// <summary>{session_flag.description}</summary>")
        lines.append(
            f'        public const string {const_name} = "{session_flag.full_key}";'
        )
    lines.append("    }")
    lines.append("")
    ...
    for enum_type in get_all_enums():
        lines.append(
            f"        public static string ToFlagValue({enum_type.__name__} value)"
        )
        lines.append("        {")
        ...
        for member in enum_type:
            pascal_name = to_pascal_case(member.name)
            lines.append(
                f'                {enum_type.__name__}.{pascal_name} => "{member.value}",'
            )
        lines.append('                _ => ""')
        lines.append("            };")
        lines.append("        }")
        ...
        lines.append(
            f"        public static {enum_type.__name__}? Parse{enum_type.__name__}(string value)"
        )
        lines.append("        {")
        lines.append("            return value switch")
        lines.append("            {")
        for member in enum_type:
            pascal_name = to_pascal_case(member.name)
            lines.append(
                f'                "{member.value}" => {enum_type.__name__}.{pascal_name},'
            )
        lines.append("                _ => null")
        lines.append("            };")
        lines.append("        }")
...
def main():
    ...
    print(f"  - {len(get_all_session_flags())} session flag keys")
