...
Generates strongly-typed C# classes from arena.builders.
...
# Import from arena package
from arena.data import Phase, Actors, PREFIX, QuestType
from arena.builders import QUEST_DEFINITIONS
...
def generate_csharp() -> str:
    ...
    lines.append("// Generated by generate_quest_data.py")
    lines.append("// Do not edit manually. Edit tools/arena/data/quests.py instead.")
    lines.append("// </auto-generated>")
    lines.append("")
    ...
    for quest in QUEST_DEFINITIONS:
        ...
        # phase is now a string (e.g., "PROLOGUE") not an enum
        phase_pascal = to_pascal_case(quest.phase) if quest.phase else "Prologue"
        advances_phase = (
            f"StoryPhase.{to_pascal_case(quest.advances_phase)}"
            if quest.advances_phase
            else "null"
        )
        quest_giver = f'"{quest.quest_giver}"' if quest.quest_giver else "null"
        # Use name_jp/name_en for Pydantic QuestDef
        display_name_jp = getattr(
            quest, "name_jp", getattr(quest, "display_name_jp", "")
        )
        display_name_en = getattr(
            quest, "name_en", getattr(quest, "display_name_en", "")
        )

        lines.append(
            f"            var q_{quest.quest_id.replace('-', '_')} = new ArenaQuestDefinition"
        )
        lines.append("            {")
        ...
        lines.append(
            f'                DisplayNameJP = "{escape_csharp_string(display_name_jp)}",'
        )
        lines.append(
            f'                DisplayNameEN = "{escape_csharp_string(display_name_en)}",'
        )
        lines.append(
            f'                Description = "{escape_csharp_string(quest.description)}",'
        )
        lines.append(f"                Phase = StoryPhase.{phase_pascal},")
        lines.append(f"                QuestGiver = {quest_giver},")
        lines.append(
            f"                AutoTrigger = {str(quest.auto_trigger).lower()},"
        )
        lines.append(f"                AdvancesPhase = {advances_phase},")
        if quest.required_quests:
            req_list = ", ".join([f'"{q}"' for q in quest.required_quests])
            lines.append(
                f"                RequiredQuests = new List<string> {{ {req_list} }},"
            )
        lines.append(f"                Priority = {quest.priority}")
        lines.append("            };")
        lines.append(
            f'            _quests["{quest.quest_id}"] = q_{quest.quest_id.replace("-", "_")};'
        )

        if quest.auto_trigger:
            lines.append(
                f"            _autoTriggerQuests.Add(q_{quest.quest_id.replace('-', '_')});"
            )

        if quest.quest_giver:
            lines.append(
                f'            if (!_npcQuests.ContainsKey("{quest.quest_giver}"))'
            )
            lines.append(
                f'                _npcQuests["{quest.quest_giver}"] = new List<ArenaQuestDefinition>();'
            )
            lines.append(
                f'            _npcQuests["{quest.quest_giver}"].Add(q_{quest.quest_id.replace("-", "_")});'
            )
